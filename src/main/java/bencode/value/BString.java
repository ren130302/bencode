...
public final class BString implements BValue<Byte[]>, Comparable<BString> {
    ...
    public static BString valueOf(String value, String encording)
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof byte[] v) {
            return Arrays.deepEquals(this.getValue(), unboxing(v));
        }
        if (obj instanceof Byte[] v) {
            return Arrays.deepEquals(this.getValue(), v);
        }
        if (obj instanceof BString v) {
            return Arrays.deepEquals(this.getValue(), v.getValue());
        }
        return false;
    }
    ...
    @Override
    public int hashCode() {
        return Arrays.deepHashCode(this.getValue());
    }
    ...
    public int compareTo(BString that) {
        ...
        if (thisLength == thatLength) {
            for (int index = 0; index < thisLength && index < thatLength; index++) {
                int sum = Byte.toUnsignedInt(this.value[index])
                        - Byte.toUnsignedInt(that.value[index]);
